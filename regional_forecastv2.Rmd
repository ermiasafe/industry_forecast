---
title: 'Industry Forecast: Regional Overview (2026 Edition)'
output:
...
region_by_industry <- vroom(here("output","industryForecast_regionShares.csv"), delim = ",") |> 
  arrange(stokes_industry, lmo_industry)
...
+ current_forecast = 3rd Cut 
+ previous_forecast = 2nd Cut 
+ CAGR for LFS (historical) 
\→ 10 Yrs MA
...
```{r}
  ...
  #                              choices = unique(employment_trend$stokes_industry),
...
industry_order <- c("Agriculture", "Oil & Gas", "Other Other Primary", "Construction", "Manufacturing",
        "Utilities", "Transportation & Warehousing", "Trade", 
        "Finance, Insurance & Real Estate", "Professional, Scientific & Technological",
        "Business, Building & Other Support Services", "Accommodation & Food Services",
        "Education Services", "Health & Social Services", "Other Services",
         "Government Services", "Total")
                    
# Application title and UI components grouped with space between them
tags$div(
  # Industry pickerInput
  shinyWidgets::pickerInput("industry1", label = "Select Stokes Industry",
...
shinyWidgets::pickerInput(
  ...
  options = list(`live-search` = TRUE,
    ...
    tags$br(),
                      ...
                      selectInput("metric", label = "Employment Metrics",
...
shinyWidgets::prettyCheckboxGroup(
  ...
  selected = setdiff(unique(employment_trend$data_type), "internal_forecast"),  # all except internal
...
checkboxInput("show_trendline", "Smoothed Trend", value = FALSE),
...
tags$p(style = "color:blue;", HTML("Toggle Individual/Shared Axis")),
...
observeEvent(input$toggle_y_axis, {
  ...
  if (y_axis_scale() == "free_y") {
    y_axis_scale("fixed")
    y_axis_limits(c(0, NA))  # fix at zero when toggling on
  } else {
    y_axis_scale("free_y")
...
observe({
  ...
  x <- region_by_industry |>
    filter(stokes_industry %in% input$industry1) |>
...
# Pre-compute STL trend reactively based on current filters
stl_trend_rv <- reactive({
  req(input$show_trendline)
  req(input$metric == "employment")  # <-- only run STL for employment metric
  
  employment_trend |>
    filter(
      lmo_industry %in% c(input$industry),
      data_type == "lfs_historical",
      bc_region %in% c(input$region)
    ) |>
    group_by(lmo_industry, bc_region, data_type) |>
    filter(n() >= 5) |>
    ungroup() |>
    as_tsibble(key = c(lmo_industry, bc_region, data_type), index = year) |>
    # FIX: Explicitly specify period = 1 for annual data so STL doesn't look for seasonality
    model(my_loess = STL(employment ~ trend() + season(period = 1))) |>
    components() |>
    as_tibble() |>
    group_by(lmo_industry, bc_region) |>          # <-- compute CAGR per group
    mutate(
      # Safeguard: only compute CAGR if there is a valid 10-year lag available
      trend_cagr10yrs = if_else(
        !is.na(lag(trend, 10)),
        scales::percent((((trend / lag(trend, 10))^(1/10)) - 1), accuracy = 0.1),
        NA_character_
      )
    ) |>
    ungroup()
...
employment_trend_rv <- reactive({
  ...
  employ_trend <- employment_trend |> 
    filter(
      stokes_industry %in% input$industry1, 
...
employ_trend_graph <- employ_trend |> 
  ...
  filter(!(input$metric == "region_share" & bc_region == "British Columbia")) |>  # <-- exclude BC for region_share
  ggplot(aes(
    x = year,
    y = .data[[input$metric]],
    group = data_type,
    ...
    text = paste0(
      "Year: ", year,
      "<br>Type: ", data_type,
      "<br>Employment: ", scales::comma(round(.data[[input$metric]], 2)),
      "<br>CAGR (10yr): ", cagr10yrs
    )
  )) + 
  geom_line()

  # Optional trendline — employment metric only
  if (input$show_trendline && input$metric == "employment") {
    stl_data <- stl_trend_rv()
    employ_trend_graph <- employ_trend_graph +
      geom_line(
        ...
        aes(
          x = year,
          y = trend,
          group = interaction(lmo_industry, bc_region),
          text = paste0(
            "Year: ", year,
            "<br>STL Trend: ", scales::comma(round(trend, 0)),
            "<br>CAGR (10yr): ", trend_cagr10yrs   # <-- trend CAGR in tooltip
          )
        ),
        colour = "gray40",
        linewidth = 0.5,
        alpha = 0.8,
        inherit.aes = FALSE   # Avoid inheriting colour/group from parent aes
      )
  }
  ...
  # Return plotly object — tooltip = "text" covers all layers including STL
  ggplotly(employ_trend_graph, tooltip = "text")
})
...
plotly::renderPlotly({
  employment_trend_rv() |>
    ...
    plotly::layout(
      margin = list(b = 10),
      hoverlabel = list(
        font = list(color = "white")  # <-- white tooltip text across all traces
      ),
      legend = list(
...
# STL for industry-faceted view (region2 filter, facets by lmo_industry)
stl_trend_ind_rv <- reactive({
  req(input$show_trendline)
  req(input$metric == "employment")  # <-- only run STL for employment metric
  ...
  employment_trend |>
    filter(
      stokes_industry %in% input$industry1,
      data_type == "lfs_historical",
      bc_region %in% input$region2        # <-- region2 not region
    ) |>
    group_by(lmo_industry, bc_region, data_type) |>
    filter(n() >= 5) |>                   # <-- drop groups with too few years
    ungroup() |>
    as_tsibble(key = c(lmo_industry, bc_region, data_type), index = year) |>
    model(STL(employment ~ trend())) |>
    components() |>
    as_tibble() |>
    group_by(lmo_industry, bc_region) |>  # <-- compute CAGR per group
    mutate(
      trend_cagr10yrs = scales::percent(  # <-- 10yr CAGR on smoothed trend
        round((((trend / lag(trend, 10))^(1/10)) - 1), 3),
        accuracy = 0.1
      )
    ) |>
    ungroup()
...
employTrend_ind_rv <- reactive({
  ...
  # Graph of the employment trend
  # Filter out 'region_share' and 'British Columbia' for industry_share before plotting
  employ_trend_graph <- employ_trend |>
    filter(input$metric != "region_share") |>
    #filter(!(input$metric == "industry_share" & bc_region == "British Columbia")) |>  # <-- exclude BC for industry_share
    ggplot(aes(
      x = year,
      y = .data[[input$metric]],
      group = data_type,
  ...
  # Optional trendline — employment metric only
  if (input$show_trendline && input$metric == "employment") {
    stl_data <- stl_trend_ind_rv()         # <-- use the industry-faceted STL reactive
    employ_trend_graph <- employ_trend_graph +
      geom_line(
        data = stl_data,
        aes(
...
```{r}
  ...
  region_industry_rv <- reactive({
    ...
    reg_by_ind <- region_by_industry |> 
      filter(stokes_industry %in% c(input$industry1), lmo_industry %in% c(input$industry)) 
...
inputPanel(
  ...
  numericInput("start_year", "Start Year", value = 2026, min = 1900, step = 1),
  numericInput("end_year", "End Year", value = 2036, min = 1900, step = 1)
)
